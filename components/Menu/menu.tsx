        ...
        {types.sort((a: any, b: any) => a.id - b.id).map((type:any) => 
          <div key={type.id}>
           ...
           <div className='lg:flex lg:flex-row'> 
              ...
              ? (
                <div className="font-serif m-5 lg:mr-36 overflow-hidden shadow-lg border border-[#6e7072] rounded-lg">
                  <div className='text-md ml-5 font-serif mt-4 lg:text-xl font-bold text-[#e9b824]'>Extras: </div>
                    <div className='mx-5 mt-6'>
                      {extras.filter((extra: any) => extra.type == type.name).sort((a: any, b: any) => a.id - b.id).map((extra: any) =>
                        <div key={extra.id} className='text-xs lg:text-base font-serif pb-3 text-[#ACADAF]'>{extra.name}: {extra.price_small} {extra.price_large}</div>
                      )}
                    </div>
                </div>
              ): (null
